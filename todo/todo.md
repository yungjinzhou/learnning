...
- 云主机列表，处理镜像列id，跳转等（已返回数据，）
- 网络列表、路由列表分页（待完成）
- 云硬盘热扩容接口测试
- 云管数据库，缓存热数据（通用方法已构建，优化工程量大，持续优化）
- 云硬盘接口热数据处理以及相关接口处理（未测试）
- 影响测试，公用一套环境，没有通知，改动对系统影响较大的服务等
- 编排模板在有编排实例时，不允许修改和删除(todo)
- 拟态下测试云硬盘创建，热 数据没有处理问题分析(已处理，数据库方法更新问题)
- 云管技术文档，
- tag修改接口处理（已处理）
...
- k8s界面问题
  ...
  - 
- - k8s问题
  - 处理minion完成后master还未完成导致的minion没有加入节点的情况（已处理）
  - 解决通过接口设置节点hostname不生效的问题（已处理）
  - 根据云管，配置controller ip映射到镜像中，解决频繁处理镜像耗时问题（已处理）
  - k8s云管接口设计（已完成）
  - 云管接口k8s模板、k8s栈创建接口开发（已完成）
  - token透传到云管设计与解决（已完成）
  - 使用kubeconfig文件方式登录
  - 需要和产品讨论，指定类似magnum界面的，k8s模板独立出来，不依赖heat原有接口，需要开发新接口
  - K8s网路互通问题（使用浮动ip访问登录界面）
  - 脚本隐藏化问题（yaml文件优化或写入镜像）
  - k8s传输安全性问题
  - k8s挂载卷问题
  - 访问方式nodeport/apiserver/userpassword
  - 版本控制 简单设计（已处理）
  - 在部署的k8s里，部署或启动服务问题
- 优化
  - 云主机，增加ip查询的方式
...
27. 创建云硬盘，选择来源，镜像，选择镜像，选中后，还是回到第一个，而不是选中的
30. 云硬盘，增加可启动列，
31. 创建实例，修改实例，实例详情，增加标签选项，tags
34. 使用中的云硬盘，可以扩容，不用置灰了
35. 云硬盘列表，状态downloading遗漏翻译： 正在下载镜像
38. 创建防火墙，在其他部门项目不显示问题，请求接口传参错误修改
28.云主机修改规格，取消不能迁移到同一主机的限制，对应host参数为要迁移的主机的名称，增加可选主机，host_name参数，参考实例迁移
33. 云主机任务状态，列表页，详情页，增加
36. 监控中心-系统状态中文化
37. 监控中心-告警管理-详情，-中文化
40.  创建云主机最终页面，显示镜像、网络
