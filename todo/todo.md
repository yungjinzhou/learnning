...
- k8s界面部署自动化
  ...
  - k8s脚本优化
- k8s界面问题
  - 在yaml文件里配置env，设置zh  zh-CN无效（参考文档里版本是kubernetes-dashboard-amd64:v1.6.3）
  - 将dashboard版本从2.0.2改成1.10.0版本测试，一直报错，镜像和apiserver结合有问题，暂未解决
  - dashbaord-amd:v1.8.3------k8s1.18，部分接口支持性不好，测试可以汉化
  - K8s-v1.14.1(1.13.1)-----dashboard-amd64:v1.10.1
  - 
  - 
- - k8s问题
  - k8s编排 节点数据互通问题
  - 脚本隐藏化问题（yaml文件优化或写入镜像）
  - k8s界面中文化问题
  - k8s传输安全性问题
  - k8s挂载卷问题
  - k8s增加浮动ip问题
  - K8s网路互通问题（浮动ip增加后，测试是否正常通信）
  - 创建时云管生成token的key，删除时云管删除该key（暂行办法）
  - 
...
26. 创建项目、创建部门、修改等操作时候，虚拟网卡更改为端口
=======
14. 放开普通用户删除卷接口,（已处理）
19. 开关设置
20. 裁决日志详情、执行体详情处理（详情，裁决策略没有翻译，异常执行体id显示不对（aarch64_8,x86_64_3，只显示了3），web容器，云控和keystone没有，可以取消显示）(已处理)
21. 云硬盘类型，改成插件式的（已处理）
22. 容器错误状态，修改，需要贺印提供新的状态信息（已处理）
23. 跟之前的一样改成插件式的，mimic，有这个参数的话，（已处理）
    1.拟态态势不显示，
    2.首页不显示右上角（云管、云控、鉴权中心这些）
    3. 首页不请求manual接口
24. 云主机列表，过滤字段增加ip，name ,镜像名称，透传到后端过滤（已处理）
25. 主机管理-主机列表(个数)，只有下一页，刷新返回第一页（已处理）
>>>>>>> 7ec0a2fc7c40f23ccf01844f54a8e516bc5c23b7

a.云硬盘，对在使用的云硬盘管理链接不可用（test_yjz项目下）（已处理）
项目配额使用情况，优化（已处理）
...
7. 网络拓扑图（已处理）
11. 网卡状态显示问题（已处理）
12. 普通用户创建容器，当没有部门流程或项目流程时，申请提示操作成功，实际接口返回400报错，（msg: "对应部门和项目的审批流程未创建或未启用"）（已处理）
13. 放开普通用户容器删除接口，容器强制删除接口，（已处理）
15. 放开普通用户云主机回收站页面，用户可以恢复或者删除（已处理）
16. 云主机详情，切换时间维度，传输参数不对（半小时-1m,24小时-5m, 一周-30m，一个月-2h）(已处理）
17. 容器挂载卷，只有已有云硬盘，（已处理）
18. 删除容器，去掉删除卷的选项，（已处理）
27. 网络类型，走接口（已处理）
3. 删除部门、删除项目，不更改字段，传参限制1个，就是不批量删除了，界面限制用户选多个，向后端传参列表中一个就行（已处理）
4. 创建项目，直接点确定，容器没有默认的个数导致报错（已处理）
5. 另外有个页面功能需要优化一下，创建主机时，选择安全组现在只能单选，建议改成可多选的形式。（已处理）

14. 放开普通用户删除卷接口,
19. 开关设置
20. 裁决日志详情、执行体详情处理（详情，裁决策略没有翻译，异常执行体id显示不对（aarch64_8,x86_64_3，只显示了3），web容器，云控和keystone没有，可以取消显示）
21. 云硬盘类型，改成插件式的
22. 容器错误状态，修改，需要贺印提供新的状态信息
23. 跟之前的一样改成插件式的，mimic，有这个参数的话，
    1.拟态态势不显示，
    2.首页不显示右上角（云管、云控、鉴权中心这些）
    3. 首页不请求manual接口
a.云硬盘，对在使用的云硬盘管理链接不可用（test_yjz项目下）
...
- 云主机-更多操作，确定哪些需要批量处理，并让前端做相应限制

<<<<<<< HEAD
- 云主机列表，列设置，默认不显示部门、项目、可用域、厂商、cpu架构、
=======
>>>>>>> 7ec0a2fc7c40f23ccf01844f54a8e516bc5c23b7
...
<<<<<<< HEAD
- 云主机列表，镜像列更改为来源，镜像值显示名称，有跳转功能
=======
>>>>>>> 7ec0a2fc7c40f23ccf01844f54a8e516bc5c23b7

- 增加镜像详情、云硬盘详情、云主机快照详情、云硬盘快照详情

- 创建中的实例，软删除失败底层处理

<<<<<<< HEAD
- 云主机详情、增加挂载卷信息、(监控信息-待定)

- 云主机详情，ip地址增加跳转，跳转到端口详情

- 云主机详情，操作日志、动作、执行结果中文化

- 云主机规格，增加元数据

- 云主机回收站，增加编辑预删除时间（需nova支持）
=======
### tmp
...
- 主机管理，增加物理内存列、增加物理cpu数量列，存储展示实际存储，（需要优化nova返回的虚拟内存数据准确性）

- 放开云硬盘类型，

- 云平台菜单可定义（交付人员权限），控制是否显示

- 分页

- - 网络
  - 云主机（可以指定当前页显示云主机数量，支持指定页数跳转）
  - 云硬盘
  - 镜像
  - 主机管理
  - 云硬盘快照
...
- 监控中心-系统状态，计算、存储、网络服务名称汉化，增加一列保留英文，默认隐藏
